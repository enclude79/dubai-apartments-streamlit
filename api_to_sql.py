    ...
    try:
        ...
        try:
            ...
            sql_query = f"""
                ...
                VALUES %s
                ON CONFLICT (id, created_at, updated_at) DO UPDATE SET
            """
...
def api_to_sql(max_records=1000, skip_csv=False, send_email=False):
    ...
    properties_data, last_updated_date = fetch_properties(max_records=max_records)
    ...
    if properties_data:
        ...
        # Сохраняем данные в CSV только если требуется (для архива)
        if not skip_csv:
            logger.info("Сохраняем данные в CSV для архива")
            csv_path = save_to_csv(properties_data)
            
        # В любом случае загружаем данные напрямую в SQL
        logger.info("Загрузка данных напрямую в базу данных")
        print("Загрузка данных напрямую в базу данных")
        ...
        if send_email:
            ...
            send_email_report(
                f"API Bayut: Загружено {new_records} новых объектов",
                f"Загрузка завершена за {duration}.\nДобавлено: {new_records}, Обновлено: {updated_records}, Ошибки: {errors}",
